...
class inotify
{
    private array $wd_constants = array(
    ...
    public function addWatchedItems(array $items): void
    {
        foreach ($items as $folderName => $folder) {
            $watchDescriptor = inotify_add_watch($this->inotifyFd, $folder, IN_CREATE | IN_DELETE | IN_MODIFY | IN_MOVED_TO | IN_MOVED_FROM);
            log::info("Monitoring directory: $folder");
            $this->watchedItemsWd[$watchDescriptor] = $folder;
            $this->watchedItemsPathsToNames[$folder] = $folderName;
        }
    }
